...
// SPEC-003 — `production-smoke` targets the live origin. When this
// project is selected we MUST NOT spin up the local `npm run generate &&
// npm run preview` web server: that would re-build the static site for
// no reason, and a generate-time failure (e.g. missing GITHUB_TOKEN)
// would block the smoke against the live origin.
const isProductionSmoke = process.env.PLAYWRIGHT_PROJECT === 'production-smoke'
...
export default defineConfig({
  ...
  projects: [
    {
      name: 'mobile',
      testIgnore: ['**/https-health.spec.ts'],
      use: {
        ...devices['iPhone 13'],
    ...
    {
      // SPEC-003 AC-13. Excluded from the default `npm run test:e2e`
      // run: invoke explicitly with
      //   PLAYWRIGHT_PROJECT=production-smoke npm run test:e2e:prod
      // The nightly-https-health workflow runs this on a cron.
      name: 'production-smoke',
      testMatch: ['**/https-health.spec.ts'],
      use: {
        baseURL: 'https://jameslanzon.com',
      },
    },
  ],
  // Local web server only when we're running the localhost-bound
  // projects. Production-smoke targets the live origin and must not
  // generate/preview the static site.
